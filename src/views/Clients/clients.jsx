...
class clients extends Component {
  ...
  openDialog() {
    this.props.htmlAlert("Seleccionar el tipo de cliente", this.openModal);
  }
  ...
  openModal(type) {
    ...
    this.setState({
      ...
      body: {
        ...
        company:
          clients.getTypeClient === "company" ? { client_code: "" } : null,
        user: [],
        contact_info: [],
        documents: [],
        comment: "",
  ...
  async setValue(name, value, modelName, mask = null) {
    ...
    if (name === "status" && this.state.client_id) {
      await clients.activeClicent(
        value === "Cliente" ? "activate" : "deactivate",
  ...
  async saveClient() {
    ...
    if (result.success) {
      this.props.alertMessage(
        ...
        "Puede verificar en la lista el nuevo cliente registrado",
        "success"
      );
      this.setState({
        create: false,
  ...
  async openDetail(id) {
    ...
    clients.clientByIdInfo.contact &&
      delete clients.clientByIdInfo.contact.updated_at;
    clients.clientByIdInfo.contact &&
      delete clients.clientByIdInfo.contact.created_at;
    clients.clientByIdInfo.user.password = "";
    if (clients.clientByIdInfo.people)
      clients.clientByIdInfo.people.owner_id = clients.clientByIdInfo.owner_id;
    if (clients.clientByIdInfo.company)
      clients.clientByIdInfo.company.owner_id = clients.clientByIdInfo.owner_id;
    if (clients.clientByIdInfo.people)
      clients.clientByIdInfo.people.related_client_id =
        clients.clientByIdInfo.related_client_id;
    this.state.errors["user"]["password"] = false;
    console.log("clients.!@@!@!@", clients.clientByIdInfo);
    let body = {
      type: type,
      people: clients.clientByIdInfo.people,
      company: clients.clientByIdInfo.company
        ? clients.clientByIdInfo.company
        : [],
      contact_info: clients.clientByIdInfo.contact
        ? clients.clientByIdInfo.contact
        : [],
      user: clients.clientByIdInfo.user ? clients.clientByIdInfo.user : [],
      documents: clients.clientByIdInfo.documents
        ? clients.clientByIdInfo.documents
        : [],
      comment: clients.clientByIdInfo.comment
        ? clients.clientByIdInfo.comment
        : "",
      authorize_data_processing:
        clients.clientByIdInfo.authorize_data_processing === 1 ? true : false,
      status: clients.clientByIdInfo.status,
      client_id: id,
      related_client_id: clients.clientByIdInfo.related_client_id
        ? clients.clientByIdInfo.related_client_id
        : null,
      owner_id: clients.clientByIdInfo.owner_id
        ? clients.clientByIdInfo.owner_id
        : null,
    };
    this.setState({
      body: body,
  ...
  async updateClient() {
    ...
    if (result.success) {
      this.props.alertMessage(
        ...
        "Puede verificar en la lista el cliente actualizado",
        "success"
      );
      this.setState({
        create: false,
  ...
  render() {
    ...
    const steps = [
      {
        name:
          clients.getTypeClient === "people"
            ? clients.fields.people.title
            : clients.fields.company.title,
        errors:
          clients.getTypeClient === "people"
        ...
        component: (
          <ClientBody
            fields={
              clients.getTypeClient === "people"
                ? clients.fields.people.fields
                : clients.fields.company.fields
            }
      ...
      {
        name: clients.fields.documents.title,
        errors: this.state.errors.documents,
        component: (
          <ClientBody
            fields={clients.fields.documents.fields}
            fieldValues={this.state.body}
            setValue={this.setValue}
            rules={clients.fields.documents.rules}
            errors={this.state.errors.documents}
            view={this.state.update ? "update" : "create"}
            alertMessage={this.props.alertMessage}
            alertLoading={this.props.alertLoading}
      ...
      <div className={!this.state.create ? "main-content" : ""}>
        {this.state.create && (
          <Wizzard
            ...
            subtitle={
              clients.getTypeClient === "people"
                ? "El cliente que esta creando es tipo persona"
                : " El cliente que esta creando es tipo empresa"
            }
            title={
              this.state.update
                ? "DETALLE DEL CLIENTE"
