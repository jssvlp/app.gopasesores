...
class InsuranceController {
  ...
  @computed
  get getListInsurances() {
    return this.listInsurances;
  }
  ...
  async getInsuranceById(id) {
    ...
    if (result.data.success && result.status === 200)
      this.InsuranceById = result.data;
    if (!result.data.success || result.status !== 200) this.InsuranceById = [];
    console.log("result", result);
  }
  async getBranchesByInsurances(id) {
    const result = await Insurances.getBranchesByInsurances(id);
    if (result.data.success && result.status === 200) return result.data;
    if (!result.data.success || result.status !== 200) return [];
    console.log("result", result);
  }
  ...
  async getInsurancesAllList() {
    const result = await Insurances.getInsurancesAll();
    if (result.data && result.status === 200) this.listInsurances = result.data;
  }
